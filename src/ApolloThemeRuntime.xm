...
static const RGBTokenEntry kDonorEntries[] = {
    ...
    // whole binary (verified via byte-pattern search), so swapping them here
    // can't hit anything else. There is no "unread" token, so route it to
    // Selection — the theme's highlight family. Apollo derives the pressed
    // state for unread rows by darkening whatever this returns (brightness
    // −0.1), so the pressed variant follows the theme automatically.
